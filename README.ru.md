...
а подсесть к ним из терминала можно с любой машины — команду целиком даёт карточка.
...
В карточке каждой сессии **два входа**: ссылка, открывающая сессию в приложении
Claude, и готовая команда подсадки, открывающая её в терминале. Telegram копирует
всё, что в `<code>`, одним тапом — поэтому продолжить на ноутбуке значит тапнуть
по команде на телефоне и вставить её там. Задан `ssh_host` в конфиге — команда
приходит вместе с `ssh`; не задан — она локальная и годится только на самой машине.

Два флага в ней стоят не случайно. `-d` отцепляет прочие **tmux**-клиенты: панель
...
с сессией через API, а не через tmux. `=rc-oms` — точное совпадение, чтобы команда
не попала в `rc-oms-2`.
...
`state_path`, `scan_depth`, `launch_timeout_s`, `ssh_host`), при перезаписи переносятся из
...
2. Заполни `config.toml`, скопировав `config.example.toml`:
   ...
   - `ssh_host` — необязательно: хост, на котором живут сессии, как в `ssh <хост>`
     (годится алиас из `~/.ssh/config` или имя в Tailscale). С ним карточки показывают
     команду подсадки целиком, вместе с `ssh`.
...
**Типы полей строгие.** `bot_token`, `worktree_root`, `state_path`, `ssh_host` — строки
(`ssh_host` может отсутствовать; пустая строка считается отсутствием);
...
- **можно подсесть с любой машины**: `ssh -t home tmux attach -d -t =rc-oms` открывает
  тот же живой терминал, которым ты управляешь с телефона. Готовую команду отдаёт
  карточка сессии, так что имя tmux-сессии выяснять не приходится.
