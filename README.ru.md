...
**Права.** `permission_mode` задаёт, с чем сессия начинает: `default` (он же `manual`),
`acceptEdits`, `plan`, `auto` или `bypassPermissions`. Значение проверяется при чтении
...
приложения — `make install-tool`), установка через `uv tool` переставляется из git,
Homebrew обновляется через `brew upgrade`, вместе с каской, если приложение стоит. Канал
определяется по пути окружения, в котором работает процесс, а не по `claude-rc` в `PATH`:
имя у всех трёх одинаковое, путь — нет.
