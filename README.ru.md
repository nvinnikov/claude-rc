...
помечены `●`), плюс `⬆️ ..`, `▶️ Start Claude RC` и `🔓 Start (bypass)` — тот же
запуск, но сразу с `bypassPermissions`. С телефона это удобнее, чем печатать
пути: тапаешь по дереву и запускаешь, не набрав ни символа. Текущий каталог
переживает перезапуск бота.

`/cd` понимает относительный путь, абсолютный, `..` и `~`.

Любая кнопка запуска сперва спрашивает ещё одно: имя сессии, ответом на
сообщение бота, или `-`, чтобы пропустить и обойтись веткой. Имя становится
ярлыком `repo@name`.

В git-репозитории рядом появляется `🌿 New worktree` — поднимает сессию в свежем
worktree с веткой вида `wt/20260730-131502`, а если сессию назвали — `wt/<имя>`.
Это способ работать параллельно с уже запущенной сессией, когда имя ветки не важно.
...
Сессия **зовётся везде одинаково**: `repo@branch` — `oms@main` для репозитория,
`oms@wt/feature-x` для worktree. Этот ярлык показывает карточка бота, его же
показывает приложение Claude и в списке сессий, и в рамке ввода (он уходит и в
`--remote-control`, и в `-n`), им же зовётся tmux-сессия до появления ссылки. Без
него каждая поверхность придумывала своё имя — приложение авто-название, карточка
имя каталога, tmux слаг, — и сводить их глазами приходилось тебе.
...
Раз сессии различают id, имени уникальность больше не нужна: карточки, `/rckill`
и `claude-rc stop` показывают и принимают ярлык `repo@branch`. Голое имя
репозитория тоже работает — `/rckill oms`, когда подняты и `oms@main`, и
`oms@wt/x`. Если цель подошла к нескольким сессиям, не гасится ничего: в ответ
приходят их id, и выбираешь ты.

### Что запущено и что осталось

`💬 Chats` показывает обе половины картины.

Сначала живые сессии — по сообщению на каждую, с кнопками `Open in Claude`,
`⏹ Stop`, `🔓 Bypass` (погасить и поднять заново с `bypassPermissions`),
`🔌 /mcp` (послать `/mcp` и показать ответ панели), `📋 Tail` (последние строки
панели) и `✏️ Rename`. Если сессия работает в worktree, в карточке видна ветка
и её состояние.
...
как было. Семь технических полей, которые визард не спрашивает (`worktree_root`,
...
2. Заполни `config.toml`, скопировав `config.example.toml`:
   ...
   - `host` — необязательно: как эта машина зовётся по ssh с других твоих машин
     (алиас из `~/.ssh/config`). Нужен только при второй машине: с ним паспорт
     сессии печатает готовую команду `ssh <host> -t 'tmux attach …'` и префикс
     `claude-rc --host <host>`
...
| `claude-rc sessions [--json] [--no-probe]` | живые RC-сессии, с состоянием и портами |
| `claude-rc whoami [путь] [--json]` | чья это сессия: ярлык, id, ссылка, команда подсадки |
| `claude-rc start [путь] [--branch ветка] [--name имя] [--new-worktree] [--resume last\|id] [--pull] [--permission-mode m]` | поднять сессию (по умолчанию — текущий каталог); `--name` даёт ярлык `repo@имя` и, вместе с `--new-worktree` без явной `--branch`, ветку `wt/<имя>` |
| `claude-rc rename <цель> <имя>` | переименовать: ярлык в tmux и `/rename` в приложении |
| `claude-rc send <цель> <текст> [--no-enter] [--tail N]` | набрать текст в панель сессии |
| `claude-rc restart <цель> [--mode m]` | погасить и поднять заново с `--resume`, при необходимости — другим режимом прав |
| `claude-rc connect [цель] [--read-only] [--cc] [--url] [--start] [--branch b]` | подсесть к панели сессии терминалом |
...
| `claude-rc --host m1 forward <цель> [порты…] [--stop]` | ssh-туннели к портам, которые слушает сессия |
| `claude-rc --host m1 <команда>` | выполнить ту же команду на другой машине по ssh (`bot`, `update`, `forward` — исключения; пути только абсолютные) |

### Для управляющего агента

Одна машина — полный продукт; добавь `host = "m1"` в конфиг, когда появится вторая, —
тогда паспорт сессии печатает готовую `ssh m1 -t 'tmux attach …'` и формы
`claude-rc --host m1 …`. Хост — любой, какой понимает ssh, включая `user@m1`. Сессии Claude, которая управляет остальными (на этой же машине
или на другой), достаточно одного вызова, чтобы увидеть всё:

```bash
claude-rc --host m1 sessions --json      # ярлык, ссылка, ветка, состояние, хвост панели, порты
claude-rc --host m1 send oms@fix '/mcp' --tail 20
claude-rc --host m1 restart oms@fix --mode bypassPermissions
claude-rc --host m1 forward oms@fix       # затем открыть http://localhost:<порт>
```

`whoami` — для агента *внутри* сессии; управляющий агент внутри не стоит, ему нужен
`sessions --json`. `state` — то, что видно на панели (`idle`, `working`, `needs_input`,
`dead`, `unknown`), а не то, что знает claude, — в сомнительных случаях читай `last_lines`.
`--no-probe` пропускает снимок панели и поиск портов ради более быстрого списка, когда
состояние не нужно. Текст для `send`, начинающийся с `-`, идёт после `--`:
`claude-rc send oms -- -x`.
...
| `clauderc/passport.py` | паспорт сессии: одна структура, рендер в HTML, текст и JSON |
| `clauderc/state_probe.py` | состояние по панели (idle/working/needs_input/dead/unknown), слушающие порты |
| `clauderc/actions.py` | действия над сессией: `send`, `tail`, `rename`, `restart` |
| `clauderc/proxy.py` | `--host`: выполнить команду на другой машине по ssh |
| `clauderc/forward.py` | ssh-туннели к портам, которые слушает сессия |
...
`callback_data` на 64 байта, защита worktree от `sync --branch`, паспорт как одно
представление, состояние как наблюдение с `unknown` по умолчанию, `@rc_mode`,
`forward` — единственная команда на клиенте, `--host` без значения), собраны в
