...
Claude, и id сессии (`🖥 rc-oms`), открывающий её в терминале. Telegram копирует
всё, что в `<code>`, одним тапом. Карточка носит id, а не команду целиком, потому
что `ssh` и хост у каждой машины свои, а меняется только id — остальное кладётся
в алиас на каждой из твоих машин:
...
Id остаются читаемыми и при совпадении имён каталогов. Два клона одного репозитория
не могут оба быть `rc-oms`, поэтому второй называется по каталогу, который их
различает: `rc-forks-oms`, а не `rc-oms-a1b2c3`. Родительских каталогов добавляется
столько, сколько нужно для уникальности, а хеш пути остаётся последним рубежом для
путей, совпавших целиком (симлинк на каталог, где сессия уже есть).
...
- **можно подсесть с любой машины**: `ssh -t home tmux attach -d -t =rc-oms` открывает
  тот же живой терминал, которым ты управляешь с телефона. Id (`rc-oms`) отдаёт карточка
  сессии, так что искать его не приходится.
