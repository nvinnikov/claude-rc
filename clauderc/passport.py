...
# Telegram в 4096 символов, и карточка не отправится вовсе. Считается по
# экранированной длине (см. `pre_block`): сырая ничего не гарантирует, потому
# что `html.escape` раздувает кавычки и скобки впятеро.
_PRE_MAX_CHARS = 1500
...
def pre_block(text: str, limit: int) -> str:
    """Текст в `<pre>` для Telegram: не длиннее `limit` уже экранированным.

    Режем сырой текст, а экранируем отрезанное: отрез после экранирования (или
    отрез уже собранной строки с тегами) попадает в середину «&lt;» или срезает
    закрывающий тег — Telegram отвечает 400, и человек не получает ничего
    вместо укороченного хвоста.

    Считаем при этом длину экранированную, а не сырую: `html.escape` раздувает
    кавычки и угловые скобки впятеро, и хвост панели, влезавший в лимит сырым,
    за него выходил — карточку резали уже снаружи, ровно тем срезом, которого
    эта функция и должна была избавить.

    Оставляем конец, а не начало: в панели последние строки и есть новость,
    а срезанное начало отмечаем многоточием.
    """
    tail = text
    # Ужимаем пропорционально: доля разметки в экранированном тексте почти
    # постоянна, поэтому шагов выходит один-два даже на сплошных кавычках.
    # `- 1` в каждом шаге гарантирует движение, если пропорция врёт.
    while True:
        cut = f"…{tail}" if len(tail) < len(text) else tail
        escaped = html.escape(cut)
        if len(escaped) <= limit or len(tail) <= 1:
            return f"<pre>{escaped}</pre>"
        tail = tail[-max(1, int(len(tail) * limit / len(escaped)) - 1) :]
