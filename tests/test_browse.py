...
from tgclaude.browse import BrowseError, change_dir, entries, is_repo


def test_entries_lists_subdirs_sorted(tmp_path: Path):
...
def test_entries_hides_dotdirs_and_heavy_dirs(tmp_path: Path):
...
def test_entries_reports_total_when_truncated(tmp_path: Path):
    for i in range(30):
...
def test_entries_on_missing_dir_raises(tmp_path: Path):
    with pytest.raises(BrowseError):
        entries(tmp_path / "nope")


def test_is_repo_detects_git(tmp_path: Path):
    (tmp_path / ".git").mkdir()
...
def test_change_dir_relative(tmp_path: Path):
    (tmp_path / "services").mkdir()

    assert change_dir(tmp_path, "services") == tmp_path / "services"


def test_change_dir_parent(tmp_path: Path):
...
def test_change_dir_absolute(tmp_path: Path):
    (tmp_path / "abs").mkdir()

    assert change_dir(Path("/"), str(tmp_path / "abs")) == tmp_path / "abs"


def test_change_dir_expands_home():
    assert change_dir(Path("/"), "~") == Path.home()


def test_change_dir_empty_keeps_current(tmp_path: Path):
    assert change_dir(tmp_path, "   ") == tmp_path


def test_change_dir_multi_segment(tmp_path: Path):
    (tmp_path / "a" / "b").mkdir(parents=True)

    assert change_dir(tmp_path, "a/b") == tmp_path / "a" / "b"


def test_change_dir_rejects_missing(tmp_path: Path):
    with pytest.raises(BrowseError, match="не найден"):
        change_dir(tmp_path, "nope")


def test_change_dir_rejects_file(tmp_path: Path):
