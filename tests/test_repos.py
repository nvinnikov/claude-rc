...
def test_discover_stops_at_depth_limit(tmp_path: Path):
...
def test_discover_skips_hidden_and_heavy_dirs(tmp_path: Path):
...
def test_discover_ignores_missing_root(tmp_path: Path):
    assert discover([tmp_path / "no-such-dir"]) == []


def test_discover_deduplicates_overlapping_roots(tmp_path: Path):
...
def test_resolve_prefers_exact_name(tmp_path: Path):
...
def test_resolve_returns_all_partial_matches(tmp_path: Path):
