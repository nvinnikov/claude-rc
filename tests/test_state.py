...
def test_remembers_across_instances(tmp_path: Path) -> None:
...
def test_falls_back_when_saved_dir_vanished(tmp_path: Path) -> None:
    gone = tmp_path / "gone"
    gone.mkdir()
...
def test_falls_back_on_broken_file(tmp_path: Path) -> None:
    path = tmp_path / "state.json"
...
def test_cwd_recovers_if_dir_disappears_at_runtime(tmp_path: Path) -> None:
    gone = tmp_path / "gone"
    gone.mkdir()
    state = State(tmp_path / "state.json", tmp_path)
    state.set_cwd(gone)
