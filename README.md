...
`🌿 Worktrees`, `⌨️ Commands`. Появляется после `/start`.

Не путать: `🌿 Worktrees` внизу — это список созданных worktree, а запускает
сессию `🌿 New worktree` внутри карточки навигации.
...
`💬 Chats` присылает по сообщению на сессию, у каждого две кнопки: `Open in Claude`
и `⏹ Stop`. Тап по `Stop` гасит именно эту сессию. Текстом — `/rckill <имя>`,
а `/rckill` без имени гасит все разом.

Worktree при этом остаётся: в нём может лежать несохранённая работа. Удаляется он
отдельно, через `/wtrm`.
