...
разошедшиеся с origin, и отсутствие upstream — не ошибка, а пропуск с причиной
в отчёте.

`--branch` переключает на ветку перед подтягиванием (существующую локально или
только в origin), но лишь у чистого репозитория. Если в каталоге прямо сейчас
работает RC-сессия, ветку там не трогают вовсе: подмена рабочего дерева под
живой сессией теряется в приложении молча (и, будь это worktree, обнулила бы
`unpushed` — см. «Грабли» в CLAUDE.md). `--no-fetch` не ходит в сеть вовсе:
если репозиторий отстал, команда об этом скажет, но подтягивать не станет —
так можно свериться со списком отставших, не тратя время на fetch по всем.
