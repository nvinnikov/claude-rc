...
- **твой Telegram user_id** — либо ввести самому, либо согласиться на автоподхват:
  ...
  с исчезнувшим каталогом в `rc_roots`), бот, скорее всего, настроен и работает,
  и визард автоподхват не предложит, чтобы не поднять второго поллера того же
  токена;
...
в файле боевой токен. Повторный `claude-rc setup` подставляет прежние значения
подсказкой и меняет только те, на которые дан новый ответ — пустой ответ оставляет
как было. Поля, которые визард не спрашивает (`worktree_root`, `scan_depth` и
другие технические настройки), при перезаписи сохраняются как есть.
