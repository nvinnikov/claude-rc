...
В git-репозитории рядом появляется `🌿 Новый worktree` — поднимает сессию в свежем
...
Под полем ввода висит постоянная клавиатура с тем, что нужно чаще всего:
`📁 Где я`, `📋 Сессии`, `📦 Репозитории`, `🌿 Worktree`, `❓ Помощь`.
Появляется после `/start`.
