...
- **токен бота** от @BotFather — сразу проверяется вызовом `getMe`, опечатка не
  дойдёт до крэш-лупа бота;
- **твой Telegram user_id** — либо ввести самому, либо согласиться на автоподхват:
  ...
  адресованного ему. Автоподхват предлагается только пока user_id ещё не задан —
  при повторном запуске рядом с уже настроенным (и, может, работающим) ботом
  визард его не предложит, чтобы не поднять второго поллера того же токена;
- **каталоги с репозиториями** через запятую (по умолчанию — `~/Documents`).

Пишет `~/.config/claude-rc/config.toml` с правами `600` — в файле боевой токен.
Повторный `claude-rc setup` подставляет прежние значения подсказкой и меняет
только те, на которые дан новый ответ — пустой ответ оставляет как было.
